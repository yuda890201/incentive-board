...
## 多言語対応（日本語 / English / नेपाली）

ログイン画面とヘッダーの言語スイッチャー（🇯🇵/EN/नेपाली ボタン）から、画面表示言語を
日本語・英語・ネパール語の3言語で切り替えられます。選択した言語は端末の`localStorage`に
保存され、次回アクセス時も引き継がれます。

- ランク名・スタッフ名・クエストやコンペのタイトル/説明/マニュアルなど、**ユーザーが入力した
  データ内容は翻訳されません**（各言語で入力した通りに表示されます）。翻訳対象は画面のラベル・
  ボタン・案内文などのUI文言のみです。
- **印刷用の集計表（🖨）は、表示言語の設定に関わらず常に日本語で出力されます。** 給与計算・
  店舗の公式記録として扱われるため、言語切り替えの対象外としています。

## ゲストモード（臨時スタッフの簡易参加）

正式にスタッフ登録されていない臨時スタッフでも、名前を入力せずにワンタップで
レシート登録・クエスト受注・コンペ応募に参加できます。

- レシート登録モーダルやコンペ応募フォームには「🧑 ゲストとして参加」ボタンがあり、
  押すと`🧑 ゲスト-XXXX`のようなランダムな識別子が名前欄に自動入力されます。
- クエスト受注時の名前入力（プロンプト）で何も入力せず送信すると、同様に自動生成された
  ゲスト名で受注されます（キャンセルした場合は受注されません）。
- ゲストとして参加した実績も、獲得表・報酬ランキングに通常のスタッフと同様に集計されます。
